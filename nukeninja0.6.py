...
# setup for the detector?? boi idk
...
game_duration = float(sys.argv[1]) if len(sys.argv) > 1 else 60.0
...
# NOTE FOR ABOVE: game duration is currently defined at the start of the script when the app is launched. line 21
...
pygame.init()
pygame.display.set_caption('Nuke-Ninja Game -- Louis-Emre-Will E11 Group')
...
def show_gameover_screen():
    ...
    # should show the spectra diagram upon game end??
    if spectra:
    ...
    if game_over:
        ...
        with CapeMCA() as mca:
            try:
                ...
                while (time.time() - start < game_duration) or (player_lives > 0):
                    # player_lives or statement added so the game ends if you die from bombs as well.
